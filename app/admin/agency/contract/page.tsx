...
import { ColumnDef, getCoreRowModel, getPaginationRowModel, getSortedRowModel, PaginationState, SortingState, useReactTable } from "@tanstack/react-table";
...
export default function Contract() {
    ...
    useEffect(()=>{
        ...
        function filterData(data: any, agencyId: string, excludeId: string) {
            // Update each row where the agency_id matches the provided agencyId
            const updatedRows: DataRow[] = data.map((row:any) => {
              if (row.agency_id === agencyId) {
                // Filter out the specific entry by excludeId within the selected agency
                const filteredData = row.data.filter((entry:any) => entry._id !== excludeId);
                return {
                  ...row,
                  data: filteredData,
              ...
              return row; // Return the row unchanged if agency_id does not match
            });
        ...
        const deleteAction=async(agency_id:string,id:string)=>{
              ...
              buttons: [
                {
                  ...
                  onClick: async()=>{ 
                    ...
                    //const updatedData:any = data.flatMap(row => 
                    //    Array.isArray(row.data) ? row.data.filter(item => item._id !== id) : []
                     // );
                    ...
                    DeleteActionGlobal({        
                      ...
                      data:{'id':id}
                        ...
                        if(deletedData.deleted_done > 0){
                          ...
                          const updatedData:any = filterData(data,agency_id,id);
                          setData(updatedData)
    ...
    useEffect(() => {
        ...
        const maxHeight = Math.max(...gridItems.map(item => item.clientHeight));
    ...
    return(
        ...
        <div className="mt-[40px]">
                <div className="flex flex-row h-[45px]">
                    <div className="py-[10px]">
                      <p className="text-[25px]  leading-[25px] capitalize  font-medium text-[#000000]">Agency Contract List</p>
                    </div>
                    ...
                    <div className="px-10">
                        <Link
                            href={'/admin/agency/contract/cu'}
                            className={`text-sm h-[45px] capitalize group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium duration-300 ease-in-out   bg-[#0166FF] bg-opacity-5 text-[#0166FF]`}
                        >
                            ...
                            <p className="text-[16px] font-medium">Add Agency Contract</p>
                        </Link>
                    </div>
                    <div className="px-5">
                        <Link
                            href={'/admin/agency/contract/ac'}
                        ...
                        >
                            ...
                            <p className="text-[16px] font-medium">Add Agency</p>
                        </Link>
                    </div>

                    <div className="px-5">
                        <Link
                            href={'/admin/agency/contract/ic'}
                            className={`text-sm h-[45px] capitalize group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium duration-300 ease-in-out   bg-[#0166FF]  text-white`}
                        >
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" width={20} height={20} strokeWidth="1.5" stroke="currentColor" className="">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                            ...
                            <p className="text-[16px] font-medium">Add Insurance</p>
                        </Link>
                    </div>
                </div>
        </div>
        ...
        {table.getRowModel().rows.map(row => (
            ...
            <CardHolder  key={row.original.agency_id} title={row.original.agency_name}>
                ...
                <table className="tanstack-table table-auto w-full text-left">
                    ...
                    <tbody>
                    {row.original.data.map((d)=>{
                        return(
                        <tr ref={el => (rowRefs.current[d._id] = el)}
                        onMouseEnter={() => handleMouseEnter(d._id)}
                        onMouseLeave={handleMouseLeave}   
                        key={d._id} className="border-t">
                            <td>{d.insurance}</td>
                            <td>{d.county}</td>
                            <td>{d.rate}</td>
                    ...
                    <GridActionLink items={[
                      ...
                      
                      {
                        actionId:'delete',
                        title:'Delete',
                        link:`delete-agencyrate`, 
                        onClick:()=>{deleteAction(row.original.agency_id,d._id)},                       
