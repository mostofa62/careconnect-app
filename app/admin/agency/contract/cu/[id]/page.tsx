...
export default function InsuranceCreate({
    ...
    const fetchDataCallback=useCallback(async()=>{
        //console.log(id);
        const response = await axios.get(`${url}agency-rate/${id}`);
        //return response.data.user;
        setFetchFormData(response.data.agencyrate);
    },[id]);
    useEffect(()=>{
        fetchDataCallback();
    ...
    const handleFormSubmit = async(values:any,{ resetForm }:any)=>{
        ...
        await axios.post(`${url}save-agency-rate/${id}`, 
            values.fetchdata, {
        ...
        <div className="mt-[40px]">
                <div className="flex flex-row h-[29px]">
                    ...
                    <div className="ml-[50px] pt-[5px]">
                        <span className="text-[25px] font-medium capitalize text-[#4F4F4F]">Update Agency Rate</span>
                    </div>
                </div>
